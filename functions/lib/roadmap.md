...
AI Model: Gemini 3.6 Flash (via @genkit-ai/googleai)
...
The codebase has just been refactored to use the httpsCallable method in Flutter to hit the local Genkit emulator. The immediate next step is to test this new connection by sending a note, verifying the local emulator processes the Genkit flow, and confirming the structured graph data lands in the live Firestore action_items collection and renders on the screen. From there, development can begin on making the UI checkboxes interactive or building the background pruning logic.